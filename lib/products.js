...
module.exports = {
    ...
    GetCompetitivePricingForSKU: function (config, params, xml, cb) {
        ...
        for (var key in params['SellerSKUList']) {
            if (/SellerSKUList.SellerSKU.[1-5]{1}/.test(key)) {
    ...
    GetCompetitivePricingForASIN: function (config, params, xml, cb) {
        ...
        if (!_.has(params, 'MarketplaceId') || !_.has(params, 'SellerASINList')) {
            cb({
                "Error": "You need to pass parameter MarketplaceId and SellerASINList"
            }, null);
            return;
        }
